...
function BuyerProductCard(props) {
  ...
  return (
    ...
    <div className={` ${classes.iconContainerFav}`} ><FontAwesomeIcon icon="fa-regular fa-heart" /></div>
...
<h5 className={`${classes.productSeller} `}><b>By</b> {product?.sellerId?.userName}</h5>
...
{product.description.length > MAX_LENGTH ?
        ...
        <div className={`${classes.productDescription}`}>{product.description}....<Link to={`${product?._id}`}>read more</Link></div>
      }



<h5 className={`${classes.productPrice} `}>{product.price.toFixed(2)} L.E</h5>
<div className={`d-flex justify-content-center mt-2 mb-4`}><button className={` btn ${classes.productButton} mx-auto px-4 mb-4`}>Add To Cart</button></div>
       </div>
    </div>
      </div>
    </>
