...
function OrderList(props) {
  ...
  function notificationStyle(orderId) {
    const style = notification.findIndex(
      (item) => item.order.read === false && item.order.orderId === orderId
    );
    return style !== -1 ? classes.notification : "";
  }
  return (
      ...
      >
        ...
        <tbody className="border-0 border-top">
          {props.listOfOrders?.map((order) => (
            <tr
              className={notificationStyle(order._id)}
              key={order?._id}
            ...
            >
              ...
              <td className="col">
                ...
                {order?.status == "canceled" && (
                  <span className={`badge mt-1 py-2 bg-dark ${classes.badge}`}>
                    Canceled
                  </span>
                )}
