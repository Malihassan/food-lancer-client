	...
	const sortSellerOrders = () => {
		if (orderCards.selectedOrderProducts) {
			for (let product of orderCards.selectedOrderProducts) {
				...
				let sellerProduct =
					seller &&
					sortedOrders[product.sellerId._id].find(
				...
				if (!seller) {
					...
					setSortedOrders({
						...
						[product.sellerId._id]: [
							...sortedOrders[product.sellerId._id],
							product,
						],
					});
					setSellerOrderPrice({
						...sellerOrderPrice,
						[product.sellerId._id]:
							sellerOrderPrice[product.sellerId._id] +
			...
			{Object.keys(sortedOrders).map((seller, idx) => {
				return (
					<div className="card d-flex flex-column text-font p-3" key={idx}>
						<button
							onClick={() => removeOrder(sortedOrders[seller], seller)}
							className="btn btn-outline-danger w-2-5 align-self-end text-center d-flex"
						>
							<FontAwesomeIcon
									...
									<li className="list-group-item d-flex flex-column">
										<div className="text-success d-flex justify-content-between">
											...
											<div className="display-6 mb-4">
												{sellerOrderPrice[seller]}&#163;
											</div>
										</div>
										<button
											onClick={() =>
												postOrder(sortedOrders[seller], seller)
											}
